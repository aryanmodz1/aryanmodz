...
def start(message):
    ...
    markup.add(KeyboardButton("馃殌 *Start Attack*"),
               KeyboardButton("鉁� *My Account*"))
    markup.add(KeyboardButton("馃攼馃攽 *Buy Key*"),
               KeyboardButton("馃毄 *Trial*"))
    bot.send_message(message.chat.id, "*Choose an option:*", reply_markup=markup, parse_mode='Markdown')

@bot.message_handler(func=lambda m: True)
def handle_menu(message):
    user_id = message.from_user.id
    text = message.text.strip().replace("*", "").replace("馃殌", "").replace("鉁�", "").replace("馃攼馃攽", "").replace("馃毄", "").strip().lower()
...
def process_attack(message):
    try:
        ...
        bot.send_message(message.chat.id, f"*Attack started 馃挜馃Ж*\n"
                                          f"*User:* {message.from_user.first_name}\n"
                                          f"*Host:* {ip}\n"
                                          f"*Port:* {port}\n"
                                          f"*Time:* {time} seconds\n\n"
                                          f"*Owner 馃憠 @team_sad001*", parse_mode='Markdown')
    except Exception as e:
        logging.error(f"Attack error: {e}")
